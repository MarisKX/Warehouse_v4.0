...
def create_on_save(sender, instance, created, **kwargs):
    ...
    if instance.c_invoice_number == "C1":
        c_invoice_count = ConstructionInvoice.objects.filter(
            constructor=instance.constructor).count()
        invoice_prefix = instance.constructor.invoice_prefix
        instance.c_invoice_number = "CI" + invoice_prefix + str(
            c_invoice_count).zfill(5)
        instance.save()
...
def update_construction_invoice_on_save_labour_costs(
